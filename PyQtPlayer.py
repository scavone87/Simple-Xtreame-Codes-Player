...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.button.clicked.connect(self.onButtonClicked)  # Associa la funzione al bottone
        ...
        self.label1.setStyleSheet("color: green; font-weight: bold; font-family: Helvetica; font-size: 14px;")
        self.label2.setStyleSheet("color: red; font-weight: bold; font-family: Helvetica; font-size: 14px;")
        self.label3.setStyleSheet("color: blue; font-weight: bold; font-family: Helvetica; font-size: 14px;")
        ...
        self.search_box_channels.textChanged.connect(self.onSearchChannelChanged)
    ...
    def onSearchChanged(self):
        ...
        for row in range(self.table1.rowCount()):
            item = self.table1.item(row, 0)  # Prendi il primo elemento della riga (puoi scegliere qualsiasi altra colonna se vuoi cercare in un'altra colonna)
            if item is not None:
    ...
    def onSearchChannelChanged(self):
        ...
        for row in range(self.table2.rowCount()):
            item = self.table2.item(row, 0)  # Prendi il primo elemento della riga (puoi scegliere qualsiasi altra colonna se vuoi cercare in un'altra colonna)
            if item is not None:
                text = item.text().lower()
                if search_text not in text:
    ...
    def onButtonClicked(self):
        ...
        if login_info[2]:
          ...
          logging.debug(f"Contenuto del login info: {server} - {username} - {password}")
          # Scarica il contenuto dell'URL
          self.progress_dialog = QProgressDialog("Caricamento in corso...", "Annulla", 0, 100, self)
          ...
          logging.debug(f"{message}, - Status: {status} - Data di scadenza: {expire_date}")
          ...
          self.progress_dialog.hide() 
            ...
            QMessageBox.warning(self, "Errore", "Il testo inserito non è un URL valido")
            self.progress_dialog.hide() 
    ...
    def populateCategoryTable(self):
        ...
        for i in range(self.df_category.shape[0]):
            self.table1.setItem(i-1,1,QTableWidgetItem(str(self.df_category.iloc[i,1])))
    ...
    def populateChannelsTable(self):
        ...
        for i in range(self.df_channels.shape[0]):
            self.table2.setItem(i-1,1,QTableWidgetItem(str(self.df_channels.iloc[i,1])))
    ...
    def populateSeriesEpisodeTable(self):
        ...
        for i in range(self.df_series.shape[0]):
            self.table3.setItem(i-1,1,QTableWidgetItem(str(self.df_series.iloc[i,0])))
